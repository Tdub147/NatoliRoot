...
// Fetches JSON's
fetch("../json/NatoliRoundTolerances.json")
  .then(response => {
...
fetch("../json/NatoliShapedTolerances.json")
  .then(response => {
    return response.json();
  })
  .then(data => {
...
fetch("../json/ISOFundamentalDeviations.json")
  .then(response => {
    return response.json();
  })
  .then(data => {
...
fetch("../json/ISOToleranceGrades.json")
  .then(response => {
    return response.json();
  })
  .then(data => {
...
function Calculate() {
  switch ($("#toleranceStyle").val()) {
    ...
    case "euTolerance":
      ...
      $("#upperTolerance").html(
        EUCalculate(
          eUStyleUpperTipDeviation.value,
          eUStyleUpperTipGrade.value,
          true
        )
      );
      $("#lowerTolerance").html(
        EUCalculate(
          eUStyleLowerTipDeviation.value,
          eUStyleLowerTipGrade.value,
          true
        )
      );
      $("#dieTolerance").html(
        EUCalculate(eUStyleDieDeviation.value, eUStyleDieGrade.value, false)
      );
      break;
    case "pmTolerance":
      $("#upperTolerance").html(
        PMCalculate(
          pmHighUpperTipToleranceSign.value,
          pmLowUpperTipToleranceSign.value,
          pmHighUpperTipTolerance.value,
          pmLowUpperTipTolerance.value,
          true
        )
      );
      $("#lowerTolerance").html(
        PMCalculate(
          pmHighLowerTipToleranceSign.value,
          pmLowLowerTipToleranceSign.value,
          pmHighLowerTipTolerance.value,
          pmLowLowerTipTolerance.value,
          true
        )
      );
      $("#dieTolerance").html(
        PMCalculate(
          pmHighDieToleranceSign.value,
          pmLowDieToleranceSign.value,
          pmHighDieTolerance.value,
          pmLowDieTolerance.value,
          false
        )
      );
      break;
...
function NatoliCalculate() {
  if (
    $("#toleranceStyle").val() == "natoliTolerance" &&
    !isNaN($("#width").val()) &&
    $("#width").val() != ""
  ) {
    // Width and Length
    if (!isNaN($("#length").val()) && $("#length").val() != "") {
      ...
      var upperClearance;
      var upperUpToSizeArray = Object.keys(
        NatoliShapedTolerances["upper"]["up_to_size"]
      ).sort(function(a, b) {
        return a - b;
      });
      for (let i = 0; i < upperUpToSizeArray.length; i++) {
        if (upperUpToSizeArray[i] >= length) {
          ...
          upperClearance =
            NatoliShapedTolerances["upper"]["up_to_size"][key]["clearance"];
          break;
        }
      }
      var lowerUpToSizeArray = Object.keys(
        NatoliShapedTolerances["lower"]["up_to_size"]
      ).sort(function(a, b) {
        return a - b;
      });
      for (let i = 0; i < lowerUpToSizeArray.length; i++) {
        if (lowerUpToSizeArray[i] >= length) {
          ...
          lowerClearance =
            NatoliShapedTolerances["lower"]["up_to_size"][key]["clearance"];
          break;
        }
      }
      ...
      upperTolerance = GetToleranceAsHTML(
        true,
        -0.0127,
        upperWidth,
        upperWidthMetric,
      ...
      lowerTolerance = GetToleranceAsHTML(
        true,
        -0.0127,
        lowerWidth,
        lowerWidthMetric,
      ...
      dieTolerance = GetToleranceAsHTML(
        false,
        0.0127,
        width,
        width * 25.4,
    ...
    // Width only
      ...
      var upperClearance;
      var upperUpToSizeArray = Object.keys(
        NatoliRoundTolerances["upper"]["up_to_size"]
      ).sort(function(a, b) {
        return a - b;
      });
      for (let i = 0; i < upperUpToSizeArray.length; i++) {
        if (upperUpToSizeArray[i] >= length) {
          key = upperUpToSizeArray[i].toString();
          upperClearance =
            NatoliRoundTolerances["upper"]["up_to_size"][key]["clearance"];
          break;
        }
      }
      var lowerUpToSizeArray = Object.keys(
        NatoliRoundTolerances["lower"]["up_to_size"]
      ).sort(function(a, b) {
        return a - b;
      });
      for (let i = 0; i < lowerUpToSizeArray.length; i++) {
        if (lowerUpToSizeArray[i] >= length) {
          key = lowerUpToSizeArray[i].toString();
          lowerClearance =
            NatoliRoundTolerances["lower"]["up_to_size"][key]["clearance"];
          break;
        }
      }
      upperWidth = width + upperClearance;
...
function GetToleranceAsHTML(
  ...
  console.log({
    GetToleranceAsHTML: {
      isShaft: isShaft,
      toleranceMetric: toleranceMetric,
      width: width,
  ...
  let toleranceSpan =
    ' <span class="frac"><sup>undefined</sup><span>&frasl;</span><sub>[undefined]</sub></span>';
  let diameterSymbol = "";
  let lengthString =
    " X " + length.toLocaleString(undefined, { minimumFractionDigits: 4 });
  let lengthStringMetric =
    " X " +
    lengthMetric.toLocaleString(undefined, { minimumFractionDigits: 4 });
  ...
  if (toleranceMetric < 0) {
    toleranceSpan =
      ' <span class="frac"><sup>+0.0000/' +
      toleranceMetric.toLocaleString(undefined, { minimumFractionDigits: 4 }) +
      "</sup><span>&frasl;</span><sub>[+0.0000/" +
      Round(toleranceMetric / 25.4, 4).toLocaleString(undefined, {
    ...
    toleranceSpan =
      ' <span class="frac"><sup>+' +
      toleranceMetric.toLocaleString(undefined, { minimumFractionDigits: 4 }) +
      "/-0.0000</sup><span>&frasl;</span><sub>[+" +
      Round(toleranceMetric / 25.4, 4).toLocaleString(undefined, {
        minimumFractionDigits: 4
      }) +
      "/-0.0000]</sub></span>";
  ...
  return (
    '<span class="frac"><sup>' +
    diameterSymbol +
    widthMetric.toLocaleString(undefined, { minimumFractionDigits: 4 }) +
    lengthStringMetric +
    "</sup><span>&frasl;</span><sub>[" +
    diameterSymbol +
    width.toLocaleString(undefined, { minimumFractionDigits: 4 }) +
    lengthString +
    "]</sub></span>" +
    toleranceSpan
  );
  ...
  if (
    ...
    if (!(inputDeviation === "0" || inputGrade === "0")) {
      ...
      if (!isNaN($("#length").val()) && $("#length").val() != "") {
        ...
        for (
          let i = 0;
          i < Object.keys(ISOFundamentalDeviations["up_to_size_mm"]).length;
          i++
        ) {
          if (
            Object.keys(ISOFundamentalDeviations["up_to_size_mm"])[i] >= length
          ) {
        ...
        deviation =
          ISOFundamentalDeviations["up_to_size_mm"][devSize][inputDeviation] *
          ISOFundamentalDeviations[type] *
          1;
        let gradeSize = "0";
        for (
          let i = 0;
          i < Object.keys(ISOToleranceGrades["up_to_size_mm"]).length;
          i++
        ) {
        ...
        tolerance =
          ISOToleranceGrades["up_to_size_mm"][gradeSize]["IT" + inputGrade];
        if (deviation < 0) {
        ...
        finalTolerance = GetToleranceAsHTML(
          isTip,
          tolerance,
          newWidth,
          newWidthMetric,
      ...
      // Doesn't have length
        ...
        for (
          ...
          if (
            Object.keys(ISOFundamentalDeviations["up_to_size_mm"])[i] >= width
          ) {
            devSize = Object.keys(ISOFundamentalDeviations["up_to_size_mm"])[i];
            break;
          }
